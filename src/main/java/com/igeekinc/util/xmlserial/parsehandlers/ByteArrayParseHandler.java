...
public class ByteArrayParseHandler implements XMLObjectParseHandler<byte []>
{
    ...
    {
    	String byteArrayStr = charInfo.toString();
    	byteArrayStr = byteArrayStr.trim();
    	returnArray = new byte[byteArrayStr.length()/2];
    	for (int curPos = 0; curPos < byteArrayStr.length(); curPos += 2)
    	{
    		int returnArrayPos = curPos/2;
    		returnArray[returnArrayPos] = (byte)Integer.parseInt(byteArrayStr.substring(curPos, curPos+2), radix);
    	}
    }
